...
import com.creditplus.p2p.common.jedis.ListTranscoder;
 ...
 *
 */
public class RedisService<M extends Serializable> {
    ...
    public boolean setList(String key, List dataList,int seconds) {  
        ...
        ListTranscoder<M> listTranscoder = new ListTranscoder<M>();
        byte[] byteArr=listTranscoder.serialize(dataList);
    ...
    public List<M> getListObj(String key){
        ...
        try {  
            ...
            resultList=listTranscoder.deserialize(value);
            return resultList;
        } catch (Exception ex) {  
            logger.error("getListObj error.", ex);  
            returnBrokenResource(shardedJedis);  
        } finally {  
            returnResource(shardedJedis);  
        }
